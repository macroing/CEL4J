...
final class ConstantLongInfoReader implements CPInfoReader {
	public ConstantLongInfoReader() {
	...
	public CPInfo readCPInfo(final DataInput dataInput, final int tag) {
		try {
			...
			throw new NodeFormatException("Unable to read CONSTANT_Long_info", e);
		}
	}
	
	@Override
	public boolean isCPInfoReadingSupportedFor(final int tag) {
		return ParameterArguments.requireRange(tag, 0, Integer.MAX_VALUE) == ConstantLongInfo.TAG;
	}
}