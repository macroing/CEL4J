...
final class ConstantDynamicInfoReader implements CPInfoReader {
	public ConstantDynamicInfoReader() {
	...
	public CPInfo readCPInfo(final DataInput dataInput, final int tag) {
		try {
			...
			throw new NodeFormatException("Unable to read CONSTANT_Dynamic_info", e);
		}
	}
	
	@Override
	public boolean isCPInfoReadingSupportedFor(final int tag) {
		return ParameterArguments.requireRange(tag, 0, Integer.MAX_VALUE) == ConstantDynamicInfo.TAG;
	}
}