...
final class ConstantMethodRefInfoReader implements CPInfoReader {
	public ConstantMethodRefInfoReader() {
	...
	public CPInfo readCPInfo(final DataInput dataInput, final int tag) {
		try {
			...
			throw new NodeFormatException("Unable to read CONSTANT_Methodref_info", e);
		}
	}
	
	@Override
	public boolean isCPInfoReadingSupportedFor(final int tag) {
		return ParameterArguments.requireRange(tag, 0, Integer.MAX_VALUE) == ConstantMethodRefInfo.TAG;
	}
}