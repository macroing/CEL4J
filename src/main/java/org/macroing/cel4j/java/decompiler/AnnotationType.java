	...
	public static AnnotationType valueOf(final Class<?> clazz) {
		...
		try {
			synchronized(ANNOTATION_TYPES) {
				final
				AnnotationType annotationType = ANNOTATION_TYPES.computeIfAbsent(clazz.getName(), name -> new AnnotationType(CLASS_FILES.computeIfAbsent(name, key -> new ClassFileReader().read(clazz))));
				annotationType.doInitialize();
